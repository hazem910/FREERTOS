...
void LCD_sendCommand(uint8_t command)
{
	...
	//softwareDelayus(1); /* delay for processing Tas = 50ns */
	Set_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
	//softwareDelayus(1); /* delay for processing Tpw - Tdws = 190ns */

	/* out the highest 4 bits of the required command to the data bus D4 --> D7 */
	LCD_DATA_PORT = (LCD_DATA_PORT & 0x0F) | (command & 0xF0);

	softwareDelayus(3); /* delay for processing Tdsw = 100ns */
	Clear_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	softwareDelayus(3); /* delay softwareDelayus processing Th = 13ns */
	Set_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
	softwareDelayus(3);/* delay for processing Tpw - Tdws = 190ns */
	...
	softwareDelayus(3); /* delay for processing Tdsw = 100ns */
	Clear_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	//softwareDelayus(1); /* delay for processing Th = 13ns */
...
void LCD_displayCharacter(uint8_t data)
{
	...
	softwareDelayus(3); /* delay for processing Tdsw = 100ns */
	Clear_BIT(LCD_CTRL_PORT,E); /* disable LCD E=0 */
	softwareDelayus(3); /* delay for processing Th = 13ns */
	Set_BIT(LCD_CTRL_PORT,E); /* Enable LCD E=1 */
	softwareDelayus(3); /* delay for processing Tpw - Tdws = 190ns */
